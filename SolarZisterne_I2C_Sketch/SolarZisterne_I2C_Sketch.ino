...
#include <Adafruit_Sensor.h>             // benötigt für Temperatur Senoren
#include <Adafruit_BME280.h>             // BME280 Temperatur Luftdruck Luftfeuchtigkeit Sensor
#include <Adafruit_BMP280.h>             // BMP280 Temperatur Luftdruck Sensor
#include "time_ntp.h"
#include "DHT.h"
...
float pfPres, pfTempIn, pfPresIn;        // Werte Luftdruck außen, Temperatur und Luftdruck innen
...
// Deklariert Temperatur Sensoren BME280 und BMP280
...
void setup() 
{
  ...
  //Einrichten der Temperatur Sensoren
  Serial.println("########################################################################################################");
  Serial.println("");
  bme.begin();
  bmp.begin();
  if (!bmp.begin()) 
  {  
    Serial.println("BMP280 Sensor konnt auf auf 0X77 gefunden werden");
  }
  else
  {
  Serial.println("Sensor zum messen des internen Temperatur(BMP280) wurden auf der Adresse 0X77 eingerichtet"); 
  }
  if (!bme.begin(0x76)) 
  {  
    Serial.println("BME280 Sensor konnt auf auf 0X76 gefunden werden");
  }
  else
  {
  Serial.println("Sensor zum messen des externen Temperatur (BME280) wurden auf der Adresse 0X76 eingerichtet"); 
  }
  Serial.println("");
  Serial.println("########################################################################################################");
  Serial.println("");
...
void loop() 
{
  ...
  if (millis()>=ulNextMeas_ms) 
  {
    ...
    pfDist[ulMeasCount%ulNoMeasValues]=((pulseIn(ECHO, HIGH)/2) / 29.1);   //Misst die Zeit von bis zur die Signal brauch vom Aussendenbis zur Rückkehr
    ...
    //Messung der externen Luftfeuchtigkeit, Temperatur und des Luftdruck
    ...
    pfPres=bme.readPressure()/100;

    //Messung der internen Temperatur und des Luftdruck
    pfTempIn=bmp.readTemperature();
    pfPresIn=bmp.readPressure()/100; 
    ...
    */
    ...
    Serial.print(pfPres);
    Serial.print("hPa - Filled: ");
    Serial.print(pfDist[ulMeasCount%ulNoMeasValues]);
    Serial.print("% - Time: ");
    Serial.println(pulTime[ulMeasCount%ulNoMeasValues]);
    Serial.print("Logging Internal Temperature: ");
    Serial.print(pfTempIn);
    Serial.print(" deg C - Pressure: ");
    Serial.print(pfPresIn);
    Serial.println("hPa ");
    Serial.print("Energie Status: ");
  ...
  if (SD.exists("LogFile.csv")) 
  {
        ...
        if (LogFile) 
        {
          ...
          LogFile.print(";");          
          String Pres = String(pfPres);
          Pres.replace(".",",");
          LogFile.print(Pres);       
          LogFile.print(";");          
          String TempIn = String(pfTempIn);
          TempIn.replace(".",",");
          LogFile.print(TempIn);    
          LogFile.print(";");
          String PresIn = String(pfPresIn);
          PresIn.replace(".",",");
          LogFile.print(PresIn);
          LogFile.print(";");
